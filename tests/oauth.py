...
class AuthTestCase(unittest.TestCase):
    ...
    def test_auth_request_token(self):
        ...
        conn = http.client.HTTPSConnection('api.500px.com')
    ...
    def test_authorization_url_with_verifier(self):
        ...
        verifier = input("Paste received oauth_verifier (blank to exit): ").strip()
        if not verifier:
            return
    ...
    def test_xauth(self):
        ...
        request_token = input("Input request token (blank to exit): ").strip()
        if not request_token:
            return

        request_token_secret = input("Input request token secret (blank to exit): ").strip()
        if not request_token_secret:
            return

        self.handler.set_request_token(request_token,request_token_secret)

        username = input("Input your username (blank to exit): ").strip()
        if not username:
            return

        password = input("Input your password (blank to exit): ").strip()
        if not password:
            return
